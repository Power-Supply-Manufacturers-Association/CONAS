...
```
schemas/
  COAS.json               top-level container: { inputs, connector, outputs }
  ...
  coas-materials.json     one general-purpose material record (conductor/plating/dielectric/elastomer)
...
data/
  coas-materials.ndjson   material registry, one JSON record per line, referenced by id
examples/
...
Runs three gates: every schema meta-validates and its `$ref`s resolve; each `examples/*.json` validates against `COAS.json`; and each example is confirmed to be a valid **PEAS** document (the `connector` branch).

## Conventions

- JSON Schema **draft 2020-12**; `$id`s under `https://psma.com/coas/`.
...
- **Material properties live once** on the referenced `coas-materials` record, never copied onto the part.
